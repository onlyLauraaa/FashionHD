...
def test_rio(test_cases, labels, model, batch_size):
    test_batch_num = int(len(test_cases) / batch_size) + 1
    f1_gnn = acc_gnn = recall_gnn = 0.0
    f1_label1 = acc_label1 = recall_label1 = 0.0
    precision_gnn = precision_label1 = 0.0
    gnn_list = []
    label_list1 = []
    ...
    for iteration in range(test_batch_num):
        ...
        # Label 指标
        label_pred = label_prob1.data.cpu().numpy().argmax(axis=1)
        f1_label1 += f1_score(batch_label, label_pred, average="macro")
        acc_label1 += accuracy_score(batch_label, label_pred)
        recall_label1 += recall_score(batch_label, label_pred, average="macro")
        precision_label1 += precision_score(batch_label, label_pred, average="macro", zero_division=0)
    ...
    # === 平均所有指标 ===
    n = test_batch_num
    f1_gnn /= n; acc_gnn /= n; recall_gnn /= n; precision_gnn /= n
    f1_label1 /= n; acc_label1 /= n; recall_label1 /= n; precision_label1 /= n
    ...
    # === 打印 ===
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    print(f"TEST GNN F1: {f1_gnn:.4f}, Acc: {acc_gnn:.4f}, Recall: {recall_gnn:.4f}, Precision: {precision_gnn:.4f}")
    print(f"TEST GNN AUC: {auc_gnn:.4f}, AP: {ap_gnn:.4f}")
    print(f"TEST Label1 F1: {f1_label1:.4f}, Acc: {acc_label1:.4f}, Recall: {recall_label1:.4f}, Precision: {precision_label1:.4f}")
    print(f"TEST Label1 AUC: {auc_label1:.4f}, AP: {ap_label1:.4f}")
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')

    # === 返回 12 个指标（平均后）===
    return (
        auc_gnn, auc_label1,
        recall_gnn, recall_label1,
        precision_gnn, precision_label1,
        f1_gnn, f1_label1,
        acc_gnn, acc_label1,
        ap_gnn, ap_label1
    )
...
def test_rio_train(train_cases, labels, model, batch_size):
    ...
    for iteration in range(test_batch_num):
        ...
        batch_nodes = train_cases[i_start:i_end]
        batch_label = labels[i_start:i_end]
        gnn_prob, _ = model.to_prob(batch_nodes, batch_label, train_flag=False)

        recall_gnn += recall_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1), average="macro")
        gnn_list.extend(gnn_prob.data.cpu().numpy()[:, 1].tolist())

    recall_gnn /= test_batch_num
    auc_gnn = roc_auc_score(labels, np.array(gnn_list))

    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    print(f"TRAIN GNN Recall: {recall_gnn:.4f}")
    print(f"TRAIN GNN AUC: {auc_gnn:.4f}")
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')

    return auc_gnn, recall_gnn  # 只返回 AUC 和 Recall